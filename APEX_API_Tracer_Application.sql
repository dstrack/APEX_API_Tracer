...
--   Date and Time:   12:03 Friday August 20, 2021
--   Exported By:     DIRK
--   Flashback:       0
...
,p_last_upd_yyyymmddhh24miss=>'20210820120300'
...
,p_last_updated_by=>'DIRK'
,p_last_upd_yyyymmddhh24miss=>'20210819234423'
);
...
,p_computation_type=>'PLSQL_EXPRESSION'
,p_computation=>wwv_flow_string.join(wwv_flow_t_varchar2(
'NVL(APEX_UTIL.GET_PREFERENCE(''API_TRACER_VARIABLE_NAME'',:APP_USER), ''lv_result'')',
''))
...
'	Nested_Table, Table_Type, Type_Object_Type',
')',
'	NOLOGGING',
'	BUILD DEFERRED',
'    REFRESH COMPLETE',
...
'    T.Item_Sequence, T.Index_By, T.Nested_Table, T.Table_Type, T.Type_Object_Type',
'FROM (        ',
'    select distinct',
'        SYN.TABLE_OWNER     Package_Owner,',
'        SYN.TABLE_NAME      Package_Name',
...
'    IS ',
'    BEGIN',
'        RETURN Enquote_Literal(p_Proce'))
);
null;
end;
/
...
'						when p_Package_Owner != cur.Package_Owner then p_Owner || ''.'' || cur.Package_Name || ''.'' ',
...
wwv_flow_api.append_to_install_script(
 p_id=>wwv_flow_api.id(261939746109436819)
,p_script_clob=>wwv_flow_string.join(wwv_flow_t_varchar2(
'*) > 1 then S.Item_Type end Record_Type,',
...
'											)',
'										|| '')''',
'									when DATA_TYPE = ''TABLE'' and RECORD_TYPE IS NOT NULL -- and NESTED_TABLE = ''Y'' ',
'										or DATA_TYPE = ''VARRAY''',
'									then',
'										''FOR idx IN 1 .. '' || ARGUMENT_NAME || ''.COUNT LOOP'' || chr(10)||RPAD('' '', 8)',
'										|| ARGUMENT_NAME || ''(idx) := ''',
